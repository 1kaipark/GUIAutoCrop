...
class SlidesManager(object):
    ...
    def create_widgets(self) -> None:
        ...
        self.tree = ttk.Treeview(
            master=self.root, columns=("fname", "padding", "crop_indices")
        )
        ...
        self.open_files_button = Button(
            master=self.root, text="open files", command=self.load_images
        )
        self.clear_files_button = Button(
            master=self.root, text="clear files", command=self.clear_files
        )
        ...
        for image_path in self.image_paths_list:
            ...
            try:
                self.tree.insert(
                    ...
                    END,
                    text=image_path,
                    values=(fname, ImageData().padding, None),
                    iid=fname,
                )
            except TclError:
                # if the IID is already taken, the image is already in the tree (unless something super weird happened.)
                pass
            self.image_data_dict[fname] = ImageData(
                image_id=fname
            )  # initialize dict with empty containers
    ...
    def update_metadata(self, image_data: "ImageData"):
        """passed to and called by the crop window"""
        ...
        if image_data.crop_indices is not None:
            crop_indices_str = ",".join([str(i) for i in image_data.crop_indices])
        else:
            crop_indices_str = None
            # can be fed list of indices to update the treeview and internal storage dict
        self.tree.item((fname,), values=(fname, image_data.padding, crop_indices_str))

    def on_item_click(self, event):
        """called when a treeview item is clicked"""
        selected_item = self.tree.selection()  # Get the selected item iid
        self.show_subwindow(selected_item)

    def show_subwindow(self, iid):
        """bring up the CropWindow"""
        ...
        self.tree.item(iid, values=(fname, padding, crop_indices))
