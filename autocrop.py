...
def crop_rect(img: np.ndarray, rect: tuple[int, int, int, int], pad: int = 25) -> np.ndarray:
...
def save_cropped_images(source_image: np.ndarray, valid_idx: list[int], brs: list[tuple[int, int, int, int]]) -> list[np.ndarray]:
