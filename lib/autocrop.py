...
def split_channels(img: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    r, g, b = (
        ...
        img[:, :, 1],
        img[:, :, 2],
    )  # split channels, output grayscale
    # return RGB images
    zero = np.zeros(
        img.shape[0:2], np.uint8
    )  # zero array w same shape as image, must be 8-bit unsigned int
...
def generate_thresholded_image(
    image: np.ndarray,
    pad: int = 50,
    k: int = 5,
    erosion: bool = False,
    ...
    contours, hierarchy = cv.findContours(
        image=t.astype(np.uint8), mode=cv.RETR_TREE, method=cv.CHAIN_APPROX_SIMPLE
    )
...
def get_cropped_images(
    ...
    brs: list[tuple[int, int, int, int]],
    pad: int = 50,
) -> list[np.ndarray]:
    """Crops image according to valid indices of bounding rects"""
    valid_rects = [brs[n] for n in crop_indices]
